...
def bootfunc(inputFunction, x, params, verbose = None):
    ...
    x - a 3D numpy array or list/tuple of 3D numpy arrays (chan x rep x time).
      ...
      each array in the list will be used in each pass of the computation;
      i.e., results will be computed by combining nPerDraw/N trials from per 
      each pool bootstrap repetition. The obvious use case is to sample an equal
      number of trials from positive and negative polarity trials.
    ...
    drawSplit = []
    distribute = int(params['nDraws']) / params['threads'] 
    leftover = int(params['nDraws']) % params['threads'] 
